...
\begin{document}
	...
	\chapter{Design}
		...
		\begin{enumerate}
			\item Boot Genode on a bare  Linux kernel without any further userland. \label{req:boot}
			No further user processes such as bash should run beside Genode.
			\item Already available drivers on Genode should be usable without any modifications.
			Furthermore the interface should support all drivers without the need to be modified itself, too.
			\item Developed kernel modules should be able to be built directly in the kernel source and as a loadable module.
		...
		\begin{enumerate}
			\item Keep the Linux kernel complexity as low as possible.
			This includes dependencies introduces by functional requirements.
			\item Remove drivers from the Linux kernel that collide with drivers available in Genode.
			If two device drivers running simultaneously access the same hardware the behaviour cannot be defined any more.
			\item Integrate the implementation into Genode in a way to keep the current and the additional functionality in parallel.
		...
		\begin{enumerate}
			\item Keep Genodes component isolation intact.
			Processes should not be able to influence each other and communicate only in ways provided by Genode.
			\item Configurable objects such as file descriptors that are created in core and passed to other components should have a trapping configuration.
		...
		%TODO explain exact constrain requirements
		...
		%TODO explain that this has been chosen and why
...
%		\section{Complexity}
%		
%		%TODO put this in a better place / rework
