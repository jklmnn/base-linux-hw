...
%		All security assumptions in the user space are invalid from this point.
	...
	\chapter{Background}
		...
		In the scope of the code the kernel has only little structure.
		It consists of a set of methods that are compiled into a single large object.
		Any method can see and therefore also call any other and is also able to access any data.
		There is little to none isolation in terms of information and execution.
		The only structure that is available are the calling conventions for procedures which have defined interfaces.
		
		To the outside the kernel interfaces via system calls.
