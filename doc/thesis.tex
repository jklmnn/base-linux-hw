...
\usepackage[margin=2.5cm,inner=3cm,foot=1.5cm]{geometry}
...
\fancyfoot[RE,LO]{\includegraphics[height=2em]{Logo.pdf}}
...
\definecolor{ComponolitGreen}{rgb}{0.0, 0.596, 0.267}
\definecolor{ComponolitDarkGray}{HTML}{706f6f}

\definecolor{LightGreen}{HTML}{009844}
\definecolor{LightYellow}{HTML}{FFF15C}
\definecolor{LightRed}{HTML}{F0AB4E}

\definecolor{findOptimalPartition}{HTML}{D7191C}
\definecolor{storeClusterComponent}{HTML}{FDAE61}

\setsansfont[Color=ComponolitDarkGray]{Arimo}
\setmainfont[Color=ComponolitDarkGray]{Arimo}
\setmonofont[Color=ComponolitDarkGray]{Cousine}

\newfontfamily\secfont[Color=ComponolitDarkGray]{Arimo Bold}

\titleformat{\chapter}[display]{\Large\secfont}{\color{ComponolitGreen}\chaptertitlename\ \thechapter}{20pt}{\Huge}
\titleformat*{\section}{\large\secfont}
\titleformat*{\subsection}{\secfont}
\titleformat*{\subsubsection}{\secfont}
...
\begin{document}
   ...
   \begin{titlepage}
      ...
      \begin{tikzpicture}[remember picture,overlay]
         \node[anchor=north east,inner sep=1.5cm] at (current page.north east) {\includegraphics[width=7cm]{Logo_Claim.pdf}};
      \end{tikzpicture}
      \par
      \vspace{7cm}
      {\Huge\noindent\textbf{Enabling Device Drivers on\\ Genode's base-linux}\\[0.5em]}
      {\Large\noindent Johannes Kliemann (kliemann@componolit.com)}
   ...
   \setlength{\parindent}{0em}
	...
	\chapter{Introduction}
		...
		The goal of this study is the combination of the advantages of both Linux and Genode.
...
<platform_info>
	...
	\chapter{Evaluation}
		...
		The working state of Genodes device drivers is the evaluation metric that this study is measured against.
		...
		As microkernels are not only defined by their functionality but also their size the Linux kernels complexity is evaluated and compared to other supported microkernels.
		
		\section{Resource requirements}
		
		To test the driver functionality the resources required by drivers need to be assessed first.
		For this task the relevant resources need to be selected and their influence on each driver must be evaluated.
		
		The resource requirements of drivers do not refer to generic resources such as CPU time and RAM but to I/O resources that enable it to communicate with hardware.
		More specific these are I/O ports, I/O memory and interrupts.
		In Genode these resources are represented by the \texttt{IO\_PORT}, \texttt{IO\_MEM} and \texttt{IRQ} sessions.
		...
		These are defined as routes in Genode.
		If they are not explicitly defined Genode applies default routes which are implicitly created from the components requirements.
...
<config>
	...
	<start name="rtc\_drv">
		...
		\begin{figure}
			\centering
			\includegraphics[width=0.8\textwidth]{framebuffer_linux_x260_scaled.png}
		...
		\begin{figure}
			...
			ylabel={Source lines of code},
                        xticklabels={
                            ...
                            \rotatebox{270}{NOVA},
                            \rotatebox{270}{seL4},
                            \rotatebox{270}{Fiasco.OC},
                            \rotatebox{270}{Fiasco.OC JDB},
                            \rotatebox{270}{Linux}},
			ymin=0,
			ymax=400000,
			area legend,
			...
			\addplot[findOptimalPartition,fill=findOptimalPartition] coordinates
                        {(0,0) (1,10944) (2, 16485) (3, 41681) (4, 70210) (5, 295741)};
			\addplot[storeClusterComponent,fill=storeClusterComponent] coordinates
                        {(0,28288) (1,27227) (2, 26807) (3, 27848) (4, 27520) (5, 28300)};
			\legend{Kernel, Base}
			\end{axis}
			\end{tikzpicture}
	...
	\chapter{Conclusion}
		...
		This study shows that it is possible to create a generic interface on Linux to run drivers in the user space.
		...
		Three basic hardware access mechanisms have been ported to Linux.
		...
		At the state in this study selecting between Genode on a bare Linux kernel and on the host system requires changes at multiple points.
